...
//This is script for handling shotgun weapon
...
public class DoubleBarrel : MonoBehaviour
{
    ...
    public Sprite DoubleBarrelHand;
    public Sprite DoubleBarrelShot;
    public float DoubleBarrelDamage;
    public float DoubleBarrelRange;
    ...
    void FixedUpdate()
    {
        ...
        if (isShot == true && ammoClipLeft > 0 && isReloading == false)
        {
            ...
            StartCoroutine("shot");
            if (Physics.Raycast(ray, out hit, DoubleBarrelRange))
            {
                hit.collider.gameObject.SendMessage("AddDamage", DoubleBarrelDamage, SendMessageOptions.DontRequireReceiver);
    ...
    IEnumerator shot()
    {
        GetComponent<SpriteRenderer>().sprite = DoubleBarrelShot;
        yield return new WaitForSeconds(0.1f);
        GetComponent<SpriteRenderer>().sprite = DoubleBarrelHand;
    }
